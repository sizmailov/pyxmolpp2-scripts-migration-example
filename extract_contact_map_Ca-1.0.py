...
print("Trajectory contain %d frames with %d chains / %d residues / %d atoms" % (len(traj), len(ref.molecules), len(ref.residues), len(ref.atoms)))
...
for frame in traj[::stride]:
    ...
    if frame.index == 0:
        h4_1_ca = frame.atoms.filter(rid_ca_1)
        h4_2_ca = frame.atoms.filter(rid_ca_2)

    for i in range(n_residues):
        for j in range(i, n_residues):
            h4_1_map[i][j] += h4_1_ca[i].r.distance(h4_1_ca[j].r)
            h4_2_map[i][j] += h4_2_ca[i].r.distance(h4_2_ca[j].r)
